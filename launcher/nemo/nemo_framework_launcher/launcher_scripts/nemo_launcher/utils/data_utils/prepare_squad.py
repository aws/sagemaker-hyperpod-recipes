...
def prepare_squad_for_fine_tuning(data_dir):
    ...
    for path, dev in path2dev.items():
        if (
            not os.path.exists(f"{os.path.splitext(path)[0]}_src.txt")
            or not os.path.exists(f"{os.path.splitext(path)[0]}_tgt.txt")
            or not os.path.exists(f"{os.path.splitext(path)[0]}_gpt.json")
        ):
...
def preprocess_squad_for_fine_tuning(fname, out_fname_prefix, dev=False):
    ...
    if (
        os.path.exists(f"{out_fname_prefix}_src.txt")
        and os.path.exists(f"{out_fname_prefix}_tgt.txt")
        and os.path.exists(f"{out_fname_prefix}_gpt.json")
    ):
        print(f'Skipped! Fine-tuning data existed at "{out_fname_prefix}*.txt"')
        if NEMO_LAUNCHER_CI:
            time.sleep(5)
        return
    with open(f"{out_fname_prefix}_src.txt", "w") as f_src, open(f"{out_fname_prefix}_tgt.txt", "w") as f_tgt, open(
        f"{out_fname_prefix}_gpt.json", "w"
    ) as f_gpt:
        for i in x["data"]:
