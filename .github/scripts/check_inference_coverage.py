...
Outputs (via GITHUB_OUTPUT):
    ...
    missing_recipes: JSON array of {run_names, js_model_id, reason, recipe_paths}
...
def main():
    ...
    # Normalize to eval-compatible schema so that resolve_model_artifacts.py
    # and workflow inline Python can use a single format (``run_names`` list,
    # ``recipe_paths`` list).  Each entry wraps exactly one recipe — inference
    # hosting configs are per-recipe, so there's no grouping.
    normalized = [
        {
            "run_names": [entry["run_name"]],
            "js_model_id": entry["js_model_id"],
            "reason": entry["reason"],
            "recipe_paths": [entry["recipe_path"]],
        }
        for entry in missing
    ]

    has_missing = len(normalized) > 0

    if has_missing:
        print(f"⚠ Found {len(normalized)} recipe(s) missing hosting configuration:\n")
        for entry in normalized:
            print(f"  Recipe: {entry['recipe_paths'][0]}")
            print(f"  run.name: {entry['run_names'][0]}")
    ...
    has_unmapped = any(entry["js_model_id"] is None for entry in normalized)

    # Set GitHub outputs — eval-compatible schema (one recipe per entry)
    _set_github_output("has_missing", str(has_missing).lower())
    _set_github_output("missing_count", str(len(normalized)))
    _set_github_output("missing_recipes", json.dumps(normalized, indent=2))
